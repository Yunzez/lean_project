...
- [~] `compiler_correct_general` — scaffolded with `induction heval`. Proven cases (17 / 22): `intr`, `nilr`, `boolr`, `varr`, `succr`, `predr`, `negtr`, `negfr`, `plusr`, `timesr`, `fstr`, `sndr`, `bindr`, `pairr`, `iftr`, `iffr`, `consr`. Remaining (`sorry` with strategy comments): `callr`, `isnilt`, `isnilf`. Added helpers: `Represents.int_inv`, `Represents.pair_inv`.
